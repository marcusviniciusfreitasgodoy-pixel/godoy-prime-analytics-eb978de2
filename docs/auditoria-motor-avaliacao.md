...
- `get_user_activity_summary`: **não existe no banco**. O hook `useUserActivityStats` a chamava e caía num fetch à view `view_user_activity_summary` com a chave publicável, sem o token do usuário. Como a view é `security_invoker` e `user_activity_logs` só libera leitura para membros autenticados, a tela de Usuários mostrava "nenhuma atividade" sempre. **Corrigido em 2026-09-02**: o hook consulta a view pelo client autenticado (tipado, com ordenação e `.limit(5000)`) e a página exibe estado de erro em vez de esconder a falha.
- Tabelas `valuation_characteristics` (49 linhas, 44 ativas) e `valuation_documentation_factors` (7 linhas): exportadas e versionadas em `supabase/migrations/20260902170000_seed_valuation_config.sql` (upsert por chave primária, idempotente). Migration aplicada em produção em 2026-09-02; contagens confirmadas após a aplicação (49 e 7), sem alteração de conteúdo. CSVs em `docs/calibracao/`. Duas observações sobre o conteúdo: `category_cap_max/min` variam entre linhas da mesma categoria (A tem 0,12/−0,08, 0,15/−0,15 e 0,15/−0,12), o que confirma a decisão de manter os caps no código (achado A8); e `display_order` repete valores, sem efeito porque a UI ordena por nome dentro da categoria. Os pesos foram editados em produção em 2026-05-20 (por exemplo, elétrica nova caiu para 0,5%); o motor lê esses pesos do banco, então já estão em uso.

### 10.8.1 Verificação final das edge functions

Em 2026-09-02, após o merge dos PRs #6 a #8, `public-itbi-stats` e `parecer-nucleo` foram reimplantadas a partir do `main`. A resposta para Copacabana / Apartamento confere com a tabela calibrada: `piso_m2` 4737, `teto_m2` 19760, `outlier_method` "mad", `engine_version` 3. Produção e repositório estão no mesmo código.
