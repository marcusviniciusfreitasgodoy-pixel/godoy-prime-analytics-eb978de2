...
| Métrica | 2026-09-03, manhã (v1.0) | 2026-09-03, 15h (recarga) | 2026-09-03, 17h12 (piso de R$ 30 mil) |
|---|---|---|---|
| Linhas em `itbi_transactions` | 30.011 | 31.472 | **33.647** (faltam cerca de 5.000 de 10 lotes perdidos, pendência 15) |
| Escrituras (`SUM(total_transacoes)`) | 125.867 | 134.555 | **141.976** |
| Período coberto | 15/01/2020 a 15/05/2026 | 15/01/2020 a 15/07/2026 | 15/01/2020 a **15/07/2026** |
| Linhas com `uso = 'Residencial'` | 100 % | 100 % | 33.619 residenciais |
| Linhas com `geom` | 30.011 | **0** na recarga; **31.369 (99,6 %)** após o backfill das 15:59 (30.137 por `logradouros_geo`, 1.232 por fallback de logradouro sem bairro, 131 nulas em ruas de volume marginal) | **33.104 de 33.619 residenciais (99,1 % das escrituras)** após novo backfill (31.820 + 1.311 fallback, 516 sem geom) |
...
| 13 | Executada, precisa repetir | **Recarga da base com o filtro novo.** Feita às 17:12 UTC com `main` em `9c12a91` (PR #20), 2020–2026, todos os bairros, recarga do período desligada, autor no `etl_log` (`435e241d`): API 43.350 agregados, 38.488 válidos, 33.500 inseridos ou atualizados, 4.428 rejeitados por percentual e 434 por faixa. Backfill de `geom` (99,1 % das escrituras), índice de preços atualizado, microrregiões da Barra reclassificadas (113 condomínios). Base: 33.647 linhas, 141.976 escrituras. **Mas 10 lotes (cerca de 5.000 linhas) falharam** por chave natural repetida (pendência 15), então a base ainda não está completa. | Implantar o PR #21 e repetir os passos 2 a 5 da sequência (carga completa idempotente; backfill de `geom` para as linhas novas; índice; 7.4, 7.4b e 7.3 em CSV). Só então regenerar o Apêndice B e reapurar a seção 16. | `etl_log` da nova carga com `registros_com_erro = 0` e `detalhes.limites_ingestao.valorMin = 30000`; linhas acima de 38.000; `geom` nulo em menos de 1 %. |
