...
-- SANTA CRUZ|Apartamento devolveram a mesma amostra em 3 e 5 anos (nenhuma
-- escritura com mais de 3 anos) e menos escrituras do que a calibracao
-- anterior a recarga. Em bairros baratos, um imovel de 40 m2 a R$ 2.400/m2
-- vale R$ 96 mil e e descartado na carga. A tabela nao guarda o que foi
-- descartado; a conferencia e feita na API da Prefeitura (mesma camada
-- ArcGIS usada pela sincronizacao), por bairro:
--   quantas features residenciais com percentual >= 90 tem valor medio
--   entre R$ 50 mil e R$ 100 mil, e quantas escrituras elas somam,
--   para GUARATIBA, INHOAIBA, SANTA CRUZ, CAMPO GRANDE, BANGU e PACIENCIA.
-- Enquanto isso, a distribuicao do que ENTROU mostra o corte: se o P1 de
-- valor_transacao de um bairro encosta em 100 mil, o filtro esta ativo la.
select bairro, tipologia,
  sum(total_transacoes) as escrituras,
  round(min(valor_transacao)) as valor_min,
  round(percentile_cont(0.01) within group (order by valor_transacao)::numeric) as valor_p1,
  round(percentile_cont(0.05) within group (order by valor_transacao)::numeric) as valor_p5,
  round(percentile_cont(0.50) within group (order by valor_transacao)::numeric) as valor_mediana,
  round(100.0 * sum(total_transacoes) filter (where valor_transacao < 110000) / sum(total_transacoes), 1) as pct_ate_110k
from itbi_transactions
where uso = 'Residencial' and percentual_transferido >= 90 and valor_m2 > 0
  and data_transacao >= current_date - interval '5 years'
group by bairro, tipologia
having sum(total_transacoes) >= 30
order by valor_p1 asc
limit 25;
