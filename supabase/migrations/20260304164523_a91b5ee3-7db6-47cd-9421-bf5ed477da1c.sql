  ...
  LOOP
    BEGIN
      ...
      IF valid_geom IS NULL OR ST_IsEmpty(valid_geom) THEN
        ...
        CONTINUE;
      END IF;
      -- Buffer by 0 to clean up any remaining topology issues
    ...
    -- Use ST_Intersects instead of ST_Within for tolerance
    BEGIN
      SELECT COUNT(*) INTO edif_count
      FROM edificacoes_geo e
      WHERE e.geom IS NOT NULL
        AND ST_Intersects(ST_Buffer(e.geom, 0), valid_geom);
    EXCEPTION WHEN OTHERS THEN
      CONTINUE;
    END;
    ...
    IF edif_count = 1
      AND (iptu_row IS NULL OR iptu_row.total_imoveis < 4)
    THEN
      BEGIN
        IF (SELECT e.area_footprint FROM edificacoes_geo e
             WHERE e.geom IS NOT NULL AND ST_Intersects(ST_Buffer(e.geom, 0), valid_geom) LIMIT 1) < 400
        THEN
          CONTINUE;
        END IF;
      EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
      ...
      FROM edificacoes_geo e
      WHERE e.geom IS NOT NULL AND ST_Intersects(ST_Buffer(e.geom, 0), valid_geom)
      ON CONFLICT DO NOTHING;

      GET DIAGNOSTICS rows_affected = ROW_COUNT;
      torres_inseridas := torres_inseridas + rows_affected;
    EXCEPTION WHEN OTHERS THEN
      NULL; -- skip torres for this lote on error
    END;
  END LOOP;
