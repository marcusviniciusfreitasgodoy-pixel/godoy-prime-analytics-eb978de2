...
BEGIN
  ...
  LOOP
    ...
    IF valid_geom IS NULL OR NOT ST_IsValid(valid_geom) OR ST_IsEmpty(valid_geom) THEN
    ...
    WHERE e.geom IS NOT NULL AND ST_Within(e.geom, valid_geom);
    ...
    IF edif_count = 1
      ...
      AND (SELECT e.area_footprint FROM edificacoes_geo e
           WHERE e.geom IS NOT NULL AND ST_Within(e.geom, valid_geom) LIMIT 1) < 400
    THEN
      CONTINUE;
    END IF;
