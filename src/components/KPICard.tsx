      ...
      <CardContent className="p-4 sm:p-5">
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1 truncate">{title}</p>
            <p className="text-2xl sm:text-3xl font-bold text-foreground truncate">{value}</p>
            {subtitle && <p className="text-xs sm:text-sm text-muted-foreground">{subtitle}</p>}
          </div>
          <div className="bg-accent/15 p-2 sm:p-3 rounded-lg border border-accent/20 ml-2 flex-shrink-0">
            <Icon className="h-4 w-4 sm:h-5 sm:w-5 text-accent" />
          </div>
        </div>
        {change && (
          <p className={`text-xs sm:text-sm font-medium ${trendColor} mb-2 truncate`}>
            {trend === "up" && "↑ "}{trend === "down" && "↓ "}{change}
          </p>
        )}
        {breakdown && (
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 pt-2 border-t border-border/50">
