...
export async function exportVideoScriptPdf() {
  ...
  const addHeader = () => {
    ...
    doc.text('Roteiro de Vídeo & FAQ - Versão 2.0', pageWidth / 2, 19, { align: 'center' });
  };
  ...
  const introText = [
    ...
    '• Lista de 40+ screenshots necessários',
    '• FAQ com 50+ perguntas frequentes em 12 categorias',
    '',
  ...
  addNarration('O Onboarding também inclui uma seção de Perguntas Frequentes com mais de 50 perguntas organizadas em 12 categorias, e busca integrada. Ao final, você pode baixar o Manual Completo em PDF, o Guia Rápido e o Roteiro de Vídeo.', pageNum);
  addScreenshot(4, 'Seção FAQ do Onboarding com accordion', pageNum);
  addScreenshot(5, 'Botões de download dos materiais de apoio', pageNum);

  // MÓDULO 1
  addTitle('MÓDULO 1: DASHBOARD (1:45 - 3:30)', pageNum);
  ...
  addNarration('Abaixo dos KPIs, temos o gráfico de Evolução de Preços com dados históricos desde 2020. Você pode alternar entre visualização Semestral ou Anual, e navegar por três abas: Geral, Por Tipologia e Variação Percentual. Logo ao lado, o gráfico de Evolução por Microbairro permite comparar a valorização de diferentes regiões ao longo do tempo.', pageNum);
  addScreenshot(8, 'Gráfico de evolução com abas visíveis', pageNum);
  addScreenshot(9, 'Gráfico de evolução por microbairro', pageNum);

  addNarration('O Ranking de Microbairros mostra quais regiões têm maior valorização ou maior liquidez. Você pode alternar entre ordenar por preço ou por volume de transações. E o novo Mapa de Transações oferece visualização geográfica de todas as transações com marcadores interativos.', pageNum);
  addScreenshot(10, 'Ranking de microbairros com toggle visível', pageNum);
  ...
  addNarration('Na aba Localização, você pesquisa transações por endereço específico. Digite o nome da rua com autocomplete inteligente, selecione tipologia, período (6 a 24 meses), faixa de área e veja os resultados com todas as transações oficiais.', pageNum);
  ...
  addNarration('Na aba Transações, você encontra logradouros por faixa de valor - de R$ 100 mil até R$ 100 milhões. Perfeito para identificar onde o mercado está mais ativo na faixa de preço do seu cliente. Exporte os resultados em Excel ou CSV.', pageNum);
  ...
  addNarration('O Motor de Avaliação Godoy Prime é um sistema completo de precificação em 6 etapas: identificação do imóvel e proprietário, localização com busca automática de dados ITBI, dados básicos como área e cômodos, questionário com 26 características em 5 categorias, resultados em três cenários (pessimista, provável e otimista), e recomendação automática com próximos passos.', pageNum);
  addScreenshot(16, 'Step 0 - Identificação do imóvel e proprietário', pageNum);
  addScreenshot(17, 'Step 1 - Localização com dados ITBI carregados', pageNum);
  addScreenshot(18, 'Step 2 - Dados básicos e escolha da base de preço', pageNum);
  addScreenshot(19, 'Step 3 - Questionário com abas de categorias', pageNum);
  addScreenshot(20, 'Step 4 - Resultados com 3 cenários, gráficos e confiança', pageNum);
  addScreenshot(21, 'Step 5 - Recomendação estratégica e próximos passos', pageNum);
  ...
  addScreenshot(23, 'Preview do PDF de avaliação', pageNum);

  // MÓDULO 4 - ESTRATÉGIA DE PRECIFICAÇÃO (NOVO)
  ...
  addScreenshot(25, 'Cards das 3 estratégias com recomendação destacada', pageNum);
  addScreenshot(26, 'Detalhes da estratégia selecionada', pageNum);

  addNarration('Para cada estratégia, você visualiza o preço de anúncio, comissão, valor líquido ao vendedor e o prêmio em relação ao valor ITBI. Há também a opção de ativar o Plano de Ajuste para reduções programadas de preço após períodos sem propostas.', pageNum);
  addScreenshot(27, 'Comparativo de valores entre estratégias', pageNum);

  // MÓDULO 5
  addTitle('MÓDULO 5: MICROREGIÕES (10:00 - 10:45)', pageNum);
  addNarration('No menu lateral, acesse Microregiões para uma análise detalhada de cada área. Cards individuais mostram o preço médio por metro quadrado, volume de transações, comparativo apartamento versus casa, e indicador de tendência. Pesquise por logradouro ou condomínio específico e compare até 5 ruas simultaneamente.', pageNum);
  addScreenshot(28, 'Página Microregiões com cards e pesquisa', pageNum);
  addScreenshot(29, 'Gráfico comparativo entre logradouros', pageNum);

  // MÓDULO 6
  addTitle('MÓDULO 6: VISTORIA DIGITAL (10:45 - 12:00)', pageNum);
  addNarration('A Vistoria Digital é seu checklist interativo para inspeção de imóveis. São mais de 55 itens para casas em 20 categorias e 50 para apartamentos em 18 categorias. Para cada item, você marca o status: OK, Atenção, Crítico, Não Verificado ou Não se Aplica. Pode adicionar observações e anexar fotos diretamente.', pageNum);
  addScreenshot(30, 'Seleção de tipo: Casa ou Apartamento', pageNum);
  addScreenshot(31, 'Checklist expandido com categorias', pageNum);
  addScreenshot(32, 'Item com status, observações e fotos', pageNum);

  addNarration('Um sistema de scoring calcula automaticamente a nota de conservação de 0 a 100. Ao finalizar, exporte o laudo PDF profissional com capa, resumo executivo, radar de diagnóstico, checklist completo e galeria de fotos. Depois, siga diretamente para a Avaliação com todos os dados pré-preenchidos.', pageNum);
  addScreenshot(33, 'Score de conservação e progresso', pageNum);
  ...
  addNarration('O módulo de Visitas oferece gestão completa do processo. No Dashboard de Visitas, acompanhe KPIs de volume, conversão e performance. Crie agendamentos com data, hora, tipo de serviço e dados do cliente. Gerencie sua disponibilidade no calendário.', pageNum);
  addScreenshot(35, 'Dashboard de Visitas com KPIs', pageNum);
  addScreenshot(36, 'Formulário de agendamento', pageNum);
  addScreenshot(37, 'Calendário de disponibilidade', pageNum);

  addNarration('Para cada visita realizada, gere uma Ficha de Visita com código único, dados completos e declaração de intermediação. Colete assinaturas digitais do cliente e corretor na tela ou envie link para assinatura remota. Envie link de feedback pós-visita por email.', pageNum);
  addScreenshot(38, 'Ficha de visita com todos os campos', pageNum);
  addScreenshot(39, 'Canvas de assinatura digital', pageNum);
  addScreenshot(40, 'Link de feedback por email', pageNum);

  addNarration('Acompanhe a evolução mensal de visitas no gráfico e veja o ranking dos corretores por volume de atendimentos.', pageNum);
  ...
  addNarration('A página Documentação é seu checklist jurídico completo para segurança nas transações. Dividida em seções para Vendedor e Comprador, com campos condicionais para PJ, União Estável ou Comunhão de Bens. O Analisador de Documentos com IA permite fazer upload de certidões e extrair informações automaticamente.', pageNum);
  addScreenshot(43, 'Checklist do Vendedor com perfil condicional', pageNum);
  addScreenshot(44, 'Checklist do Comprador', pageNum);
  addScreenshot(45, 'Analisador de Documentos IA com resultados', pageNum);

  // MÓDULO 9
  addTitle('MÓDULO 9: SOFIA - ASSISTENTE IA (14:30 - 15:15)', pageNum);
  addNarration('No canto inferior direito, você encontra Sofia - nossa Assistente de Mercado com Inteligência Artificial. Faça perguntas em linguagem natural sobre preços, tendências, comparativos entre bairros, documentação ou qualquer dúvida sobre a plataforma. Sofia consulta a base de dados em tempo real e oferece sugestões inteligentes. Use comandos de voz para interação hands-free e ouça as respostas em áudio.', pageNum);
  addScreenshot(46, 'Botão da Sofia no Dashboard', pageNum);
  addScreenshot(47, 'Chat aberto com pergunta e resposta contextual', pageNum);
  addScreenshot(48, 'Análise de documento pela Sofia', pageNum);

  // MÓDULO 10
  addTitle('MÓDULO 10: RECURSOS ADMINISTRATIVOS (15:15 - 16:15)', pageNum);
  addNarration('Administradores têm acesso a recursos avançados: Base de Conhecimento para gerenciar conteúdo da IA, Calibrador de Avaliação para ajustar pesos das características, Gestão de Leads com filtros e exportação, Gerenciamento de Usuários com papéis e permissões, Rastreamento de Atividades para auditoria completa, e Sincronização de dados ITBI da Prefeitura.', pageNum);
  addScreenshot(49, 'Página de Gestão de Leads', pageNum);
  ...
  addNarration('Todas as análises podem ser exportadas. No Dashboard, exporte KPIs em Excel ou faça Backup Completo. Avaliações, Vistorias, Fichas de Visita e Documentação geram PDFs profissionais. Na página de Onboarding, baixe o Manual Completo, o Guia Rápido e o Roteiro de Vídeo.', pageNum);
  addScreenshot(52, 'Menu de exportação do Dashboard', pageNum);
  addScreenshot(53, 'PDFs gerados pela plataforma', pageNum);

  // ENCERRAMENTO
  addTitle('ENCERRAMENTO (16:45 - 17:30)', pageNum);
  addNarration('O Godoy Prime Analytics é mais do que uma ferramenta - é seu parceiro estratégico no mercado imobiliário de alto padrão. Dados oficiais ITBI, avaliações automatizadas com metodologia própria, estratégia de precificação inteligente, vistorias digitais completas, gestão de visitas com assinatura digital, documentação de due diligence, assistente com inteligência artificial e onboarding interativo - tudo em uma única plataforma. Acesse agora e transforme dados em resultados. Godoy Prime Realty - Inteligência que valoriza seu negócio.', pageNum);
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('50+ perguntas organizadas em 12 categorias', pageWidth / 2, y, { align: 'center' });
  y += 12;

  // CATEGORIA 1: GERAL
  addTitle('GERAL', pageNum);
  addFaqItem(1, 'O que é o Godoy Prime Analytics?', 'É a plataforma de inteligência imobiliária mais completa do Rio de Janeiro, oferecendo análise de dados oficiais ITBI, avaliações automatizadas, vistorias digitais, gestão de visitas e assistência por IA.', pageNum);
  ...
  addFaqItem(3, 'A plataforma funciona em dispositivos móveis?', 'Sim, a interface é totalmente responsiva e funciona em desktops, tablets e smartphones. Pode ser instalada como PWA.', pageNum);
  addFaqItem(4, 'Preciso instalar algum software?', 'Não, a plataforma funciona diretamente no navegador web. Opcionalmente, pode ser instalada como PWA para acesso rápido.', pageNum);

  // CATEGORIA 2: DASHBOARD
  addTitle('DASHBOARD E INDICADORES', pageNum);
  addFaqItem(5, 'Com que frequência os dados são atualizados?', 'Os dados ITBI são sincronizados mensalmente com as bases oficiais da Prefeitura do Rio de Janeiro.', pageNum);
  ...
  addFaqItem(7, 'Como funciona o ranking de microbairros?', 'Os microbairros são ordenados pela mediana de preço por m² ou volume de transações, permitindo identificar regiões mais valorizadas ou líquidas.', pageNum);
  addFaqItem(8, 'Posso exportar os gráficos do dashboard?', 'Sim, você pode exportar relatórios completos em PDF, dados em Excel/CSV e fazer backup completo do banco de dados.', pageNum);

  // CATEGORIA 3: PESQUISAS
  addTitle('PESQUISAS DE MERCADO', pageNum);
  addFaqItem(9, 'Quais filtros estão disponíveis nas pesquisas?', 'Localização (bairro, logradouro), faixa de valor (R$ 100 mil a R$ 100 milhões), período (6 a 24 meses), área em m² e tipologia.', pageNum);
  ...
  addFaqItem(15, 'Como é calculado o nível de confiança?', 'Baseado na quantidade de transações ITBI disponíveis para o logradouro e na consistência das características avaliadas.', pageNum);
  ...
  addFaqItem(22, 'Qual a diferença entre vistoria de casa e apartamento?', 'Casas têm 55+ itens em 20 categorias incluindo área externa. Apartamentos têm 50+ itens em 18 categorias focadas em áreas comuns e privativas.', pageNum);
  addFaqItem(23, 'Como funciona o sistema de scoring?', 'Cada item é avaliado de 1 (Crítico) a 5 (Excelente). O sistema calcula um score de conservação de 0 a 100.', pageNum);
  addFaqItem(24, 'Posso anexar fotos à vistoria?', 'Sim, você pode registrar fotos para documentar cada item avaliado, com upload direto pelo celular ou computador.', pageNum);
  addFaqItem(25, 'O relatório de vistoria serve como laudo técnico?', 'O relatório serve como documentação detalhada, mas laudos técnicos oficiais requerem profissional habilitado.', pageNum);
  ...
  addFaqItem(28, 'Como funciona o feedback pós-visita?', 'Após a visita, um link é enviado ao cliente para avaliar o imóvel, informar interesse e registrar observações.', pageNum);
  addFaqItem(29, 'Posso ver estatísticas de visitas?', 'Sim, o dashboard mostra KPIs de volume, conversão, evolução mensal e ranking dos corretores.', pageNum);

  // CATEGORIA 8: DOCUMENTAÇÃO
  addTitle('DOCUMENTAÇÃO', pageNum);
  addFaqItem(30, 'Quais documentos são verificados no checklist?', 'Documentos do imóvel (matrícula, IPTU), do vendedor/comprador (RG, CPF, certidões) e da transação (contrato, procurações).', pageNum);
  addFaqItem(31, 'Como funciona o analisador de documentos por IA?', 'Você faz upload do documento e a IA identifica automaticamente o tipo e extrai informações relevantes.', pageNum);
  addFaqItem(32, 'Posso usar o checklist para qualquer tipo de transação?', 'Sim, o checklist é adaptável para compra, venda, locação e outras operações imobiliárias.', pageNum);
  ...
  addFaqItem(37, 'Posso usar comandos de voz?', 'Sim, a Sofia aceita consultas por voz e pode responder com síntese de voz.', pageNum);

  // CATEGORIA 10: ADMINISTRATIVO
  addTitle('RECURSOS ADMINISTRATIVOS', pageNum);
  addFaqItem(38, 'Como gerenciar leads capturados?', 'Acesse a seção Leads para visualizar, filtrar por origem/interesse/período e acompanhar o status de cada prospect.', pageNum);
  ...
  addFaqItem(41, 'O que é rastreado no log de atividades?', 'Logins, avaliações, vistorias, visitas, pesquisas, exportações e todas as ações relevantes.', pageNum);
  ...
  addFaqItem(43, 'Existe treinamento disponível?', 'Sim, oferecemos onboarding interativo, tours guiados, manual em PDF, guia rápido e roteiro de vídeo.', pageNum);
  addFaqItem(44, 'Como reportar um bug ou erro?', 'Entre em contato pelo email de suporte descrevendo o problema detalhadamente.', pageNum);
  ...
  addFaqItem(46, 'Qual a melhor forma de começar a usar a plataforma?', 'Complete o onboarding, explore o dashboard, faça uma avaliação teste e uma vistoria para conhecer os fluxos.', pageNum);
  ...
  addFaqItem(48, 'Posso usar a plataforma offline?', 'Não, é necessária conexão com internet para acessar dados em tempo real e funcionalidades da IA.', pageNum);
